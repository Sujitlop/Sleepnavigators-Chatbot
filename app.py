...
if app_mode == "💬 Patient Concierge":
    # Restrict maximum width for a cleaner, centered chat appearance
    ...
    with chat_col:
        ...
        # Initialize chat history in Streamlit's browser session memory
        if "messages" not in st.session_state:
            st.session_state.messages = [
                {"role": "assistant", "content": get_bot_intro()}
            ]
        
        # Keep historical chat logs visible on the screen during page refreshes
        for msg in st.session_state.messages:
            with st.chat_message(msg["role"]):
                st.write(msg["content"])
        
        # Capture user typing from the interactive chat input bar
